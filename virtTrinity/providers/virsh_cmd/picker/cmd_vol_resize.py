...
from virtTrinity.providers.virsh_cmd.picker.cmds import CmdVolResizeChecker
from virtTrinity.providers.virsh_cmd import data


class CmdVolResizePoolVolumePairPicker(picker.Picker):
    depends_on = CmdVolResizeChecker
...
class CmdVolResizeVolPicker(picker.Picker):
    depends_on = CmdVolResizeChecker
...
class CmdVolResizePoolPicker(picker.Picker):
    depends_on = CmdVolResizeChecker
    data_type = common_data.String()
    types = {
        "positive": {
...
class CmdVolResizeCapacityPicker(picker.Picker):
    depends_on = CmdVolResizeChecker
    data_type = common_data.String()
    types = {
        "positive": {
